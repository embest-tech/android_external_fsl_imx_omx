...
OMX_ERRORTYPE GMInbandSubtitleSource::SetType(OMX_STRING type)
{
    ...
    printf("GMInbandSubtitleSource set type: %s\n", type);
...
OMX_ERRORTYPE SRTParser::SeekTo(OMX_TICKS nPos)
{
    ...
    LOG_DEBUG("SRTParser want seek to %lld us, actually to %lld, offset: %lld\n", 
            nPos, mTimeTable[index].nTime, mTimeTable[index].nOffset);
...
OMX_ERRORTYPE SRTParser::BuildSeekTable()
{
    ...
    while(1) {
        ...
        if(mTimeTableEntrySize >= mTimeTableSize) {
            OMX_PTR temp = FSL_MALLOC(mTimeTableSize + SRT_TIME_TABLE_SIZE);
            if(!temp)
                break;
            fsl_osal_memcpy(temp, mTimeTable, mTimeTableEntrySize);
            FSL_FREE(mTimeTable);
            mTimeTable = (TIME_NODE*)temp;
            mTimeTableSize += SRT_TIME_TABLE_SIZE;
        }
        LOG_DEBUG("time: %lld, offset: %lld\n", node->nTime, node->nOffset);
    }
